        ...
        <div className="flex flex-col gap-4">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="flex justify-between items-center py-2 border-b border-gray-50">
              <div className="h-4 bg-gray-50 rounded w-28"></div>
              <div className="h-4 bg-gray-50 rounded w-10"></div>
            </div>
          ))}
        </div>
      </div>
  ...
  if (!data || data.length === 0) {
    return (
      <div className="bg-white p-4 rounded-xl shadow flex flex-col items-center justify-center min-h-[200px] text-gray-400">
        <h3 className="text-lg font-bold mb-2 text-gray-800 self-start">Low Stock Alerts</h3>
        <p className="text-sm">All product stocks are above thresholds.</p>
      </div>
    );
  }
        ...
        {data.map((product) => (
          <div key={product.productId} className="flex justify-between items-center py-2 border-b last:border-0 border-gray-100">
            ...
            <span className="font-medium text-amber-600 shrink-0">
              {product.stock} {product.threshold ? `/ ${product.threshold}` : ""}
            </span>
          </div>
        ))}
      </div>
